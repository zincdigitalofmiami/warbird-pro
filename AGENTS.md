...
- The local Optuna operator surface is canonical on `http://localhost:8090` only (the 8080 compatibility redirect was retired 2026-04-23). Treat on-disk directories under `scripts/optuna/workspaces/` as the source of truth for live lanes. Registry entries without a real workspace directory are not active, and child study dashboards launch on demand.
...
- **v8 LOCK — `indicators/v8-warbird-live.pine` and `indicators/v8-warbird-prescreen.pine`
  are CODE-FROZEN. The ONLY permitted changes are `input.*` default value adjustments
  for settings optimization. No structural edits, no new blocks, no geometry changes,
