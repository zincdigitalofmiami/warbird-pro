#!/usr/bin/env python3
"""
Poll Newsfilter query API into normalized raw research tables as Warbird's primary news source.

Uses the official search endpoint plus the official article-content endpoint so
the stored body text is strict article content instead of page chrome.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Poll Newsfilter into raw research tables.")
...
def require_newsfilter_key() -> str:
    load_project_env()
    api_key = os.environ.get("NEWSFILTER_API_KEY")
    if not api_key:
        raise RuntimeError("NEWSFILTER_API_KEY is required for Newsfilter polling.")
    return api_key
...
def fetch_articles(api_key: str, lookback_days: int, size: int) -> list[dict[str, Any]]:
    contract = load_raw_news_contract()
    start_date = (datetime.now(timezone.utc) - timedelta(days=lookback_days)).strftime("%Y-%m-%d")
    source_clause = " OR ".join(sorted(contract.newsfilter_source_allowlist))
    payload = {
        "queryString": f"publishedAt:[{start_date} TO *] AND source.id:({source_clause})",
        "from": 0,
        "size": size,
    }
...
def fetch_article_html(api_key: str, article_id: str) -> str:
    response = requests.get(
        f"https://api.newsfilter.io/articles/{article_id}.html",
...
def upsert_articles(supabase: Client, rows: list[dict[str, Any]]) -> dict[str, int]:
    if not rows:
        return {}
    supabase.table("econ_news_newsfilter_articles").upsert(rows, on_conflict="dedupe_key").execute()
    id_by_dedupe: dict[str, int] = {}
    for keys_chunk in chunked([row["dedupe_key"] for row in rows], 200):
        response = (
            supabase.table("econ_news_newsfilter_articles")
...
def upsert_segment_links(
    ...
    if not payload:
        return 0
    supabase.table("econ_news_newsfilter_article_segments").upsert(
        payload,
...
def print_samples(samples: list[dict[str, Any]], sample_limit: int) -> None:
    if sample_limit <= 0:
        return
    print()
    print("Sample Newsfilter articles:")
...
def run_pass(args: argparse.Namespace, supabase: Client | None) -> tuple[int, int, int]:
    contract = load_raw_news_contract()
    api_key = require_newsfilter_key()
    topic_codes = args.topics or list(contract.topic_codes)
    topic_map = contract.topic_map

    rows = fetch_articles(api_key, args.lookback_days, args.size)
    print(f"Fetched {len(rows)} Newsfilter articles")
    ...
    for item in rows:
        ...
        if source_id not in contract.newsfilter_source_allowlist:
            continue
        ...
        symbols = [normalize_text(symbol).upper() for symbol in item.get("symbols", []) if normalize_text(symbol)]
        matched_symbols = [symbol for symbol in symbols if symbol in contract.watchlist_symbols]
        if not matched_symbols:
            continue

        publisher_domain = extract_domain(source_url)
        if publisher_domain and publisher_domain not in contract.trusted_domains:
            continue
        ...
        article_html = fetch_article_html(api_key, article_id)
        article_body = strip_html_tags(article_html)
        body_word_count = len([token for token in article_body.split() if token])
        extraction_status = "FULL" if body_word_count >= 120 else "PARTIAL" if body_word_count > 0 else "FAILED"
        if extraction_status == "FAILED":
            extraction_failures += 1

        article_key = build_article_key(article_id, source_url, title, published_at)
        image_url = normalize_text(item.get("imageUrl") or item.get("image")) or None

        article_rows[dedupe_key] = {
            "article_key": article_key,
            "provider": "newsfilter",
            "newsfilter_id": article_id,
            "source_id": source_id,
            "source_name": normalize_text(source.get("name")) or source_id,
            "url": source_url,
        ...
        for topic_code in topic_codes:
            ...
            matched_topic_keywords = matched_keywords(title, f"{summary or ''} {article_body}", topic.keywords)
...
def main() -> None:
    ...
    try:
        ...
        print(f"Newsfilter raw intake | target={args.target} | mode={mode} | dry_run={args.dry_run}")
