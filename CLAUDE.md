...
- `indicators/v7-warbird-institutional.pine` is the active Pine work surface. Compiles clean. Current verification 2026-04-26: pine-facade compile clean; `pine-lint.sh` passes with W4/W9 only; output budget is 58/64 (55 plot + 3 alertcondition, 6 headroom). 4 `request.security()` calls + 1 `request.footprint()` call (live, implemented).
- `indicators/v7-warbird-strategy.pine` is the v7 Strategy Tester / AG export-compatibility surface, not the canonical local AG ingestion path. Current verification 2026-04-26: pine-facade compile clean; `pine-lint.sh` passes with W9 only; output budget is 60/64 (60 plot, 4 headroom); v7 indicator/strategy parity guard passes. Commission floor at $1.00/side. `use_bar_magnifier=true`, `slippage=1`, and `process_orders_on_close=false` are pinned in `strategy()`. Actual strategy entries follow the gated SETUP -> accept/scalp confirmation path; do not treat strategy orders as "every fib touch" training rows without a contract repair.
...
- `indicators/v8-warbird-live.pine` — locked legacy v8 baseline, code-frozen. presetInput="Custom". calcScoreBreakdown/calcSignalScore hoisted unconditionally (Pine v6 CW10003/CW10004). pine-facade + TV smart_compile clean. Commit cd5cbd5 (2026-04-17).
...
- Phase 4 bootstrap pipeline now implemented (`scripts/ag/build_ag_pipeline.py`) and executed on local `warbird`. Latest audited local shape (2026-04-26): `ag_fib_snapshots=2,240`, `ag_fib_interactions=54,662`, `ag_fib_stop_variants=327,972`, `ag_fib_outcomes=327,972`, `ag_training=327,942` rows. `ag_training` is above the 327,000 row floor, spans `2020-01-02 20:00 America/Chicago` through `2026-04-13 09:45 America/Chicago`, and has six balanced stop-family surfaces of 54,657 rows each.
...
- Repo-native baseline trainer now exists (`scripts/ag/train_ag_baseline.py`). It joins the canonical `ag_training` rows with the curated `FRED/econ_calendar` regime context, enforces the required series set in the FRED catalog, removes label leakage from `ag_training`, writes run artifacts under `artifacts/ag_runs/`, and blocks training when a validation/test slice has fewer than 2 target classes.
  ...
  - 2026-04-26 run reconciliation: stale `RUNNING` row `agtrain_20260419T121509617194Z` was marked `FAILED` in local `warbird` with `fold_count=2`. There is no active `train_ag_baseline` / AutoGluon process and no remaining `RUNNING` rows. The partial artifact has complete fold summaries only for folds 01-02, predictor artifacts without summary for fold 03, no folds 04-05, no SHAP, and no Monte Carlo; it is not a model-selection candidate.
...
- TradingView enforces a hard maximum of 64 output calls per script. Current v7 budget: institutional 58/64 (6 headroom), strategy 60/64 (4 headroom), institutional backtest strategy 53/64 (11 headroom).
