...
- v7 intermarket basket: **TICK** (`USI:TICK`), **VOLD** (`USI:VOLD`), **VVIX** (`CBOE:VVIX`), **VIX/VIX3M** term structure (`CBOE:VIX` + `CBOE:VIX3M`), **HYG** (`AMEX:HYG`), **RTY** (`CME_MINI:RTY1!`). All intraday flow-based leading indicators. **SKEW** (`CBOE:SKEW`) and **NYSE A/D** (`USI:ADD`) are daily-only context exports, NOT gate members.
- v7 regime: grouped weighted scoring (flow 30% + vol 45% + participation 15% + execution 10%) → 0-100 score with hysteresis (bull >65, bear <35, exit at 50). Persistence (confirmBars), cooldown, neutralize. Override (direct bull↔bear flip) only when flow AND vol both extreme same direction. No unanimous gate, no decision model dropdown.
- TICK and VOLD are flow indicators (positive/negative threshold), not price instruments (no EMA trend). HYG and RTY use EMA trend. VTS ratio and VVIX use level thresholds. VVIX/VTS inverted at group boundary (low VVIX = positive = risk-on).
...
- Pivot distance/state is a critical trigger and reversal input, but not the sole decision maker. Intermarket basket is 6 intraday flow-based LEADING indicators (TICK, VOLD, VVIX, VIX/VIX3M term structure, HYG, RTY) grouped into flow/vol/participation/execution scores. SKEW and NYSE A/D are daily-only context exports, not gate members. AG decides correlations and weights from data.
...
- Minimal Pine export surface for training capture: fib lines/state, TA core pack (EMAs/MACD/RSI/ATR/ADX/volume family/OBV/MFI), intermarket state (TICK/VOLD/VVIX/VTS/HYG/RTY + daily SKEW/ADD), ES execution quality (VWAP/range/efficiency), regime score, agreement velocity, impulse quality, and event/regime state from the canonical indicator surface (`v7-warbird-institutional.pine`).
- TradingView enforces a hard maximum of 64 output calls per script. Hidden `display.none` plots AND `alertcondition()` calls both count toward the cap. Current v7 budget: 61 plot + 3 alertcondition = 64/64 (0 headroom). TA Core Pack exports (7 plots) are AG server-side computable — cut candidates if slots needed.
...
- The current blocking sequence is: ~~Pine indicator recovery~~ (DONE) -> ~~v7 institutional upgrade~~ (DONE, 64/64, TV-validated, flow-based intermarket + grouped regime + ES execution + 6 new exports) -> fib engine hardening (anchor span, waypoint lines) -> canonical writer cutover -> dashboard/admin reader cutover -> training workbench buildout -> legacy retirement.
