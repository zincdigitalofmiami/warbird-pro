...
- The current blocking sequence is: ~~Pine indicator recovery~~ (DONE) -> ~~v7 institutional upgrade~~ (DONE, 64/64) -> **intermarket pivot to CME Globex** (IN PROGRESS: data backfill running, Pine update pending) -> fib engine hardening (anchor span, waypoint lines) -> canonical writer cutover -> dashboard/admin reader cutover -> training workbench buildout -> legacy retirement.
